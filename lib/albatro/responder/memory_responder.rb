...
module Albatro
  module Responder
    ...
    # 指定したメッセージを順に発言する
    #
    #  responder = Albatro::Responder::MemoryResponder.new(:messages => ["1", "2"])
    #  p responder.dialogue("") #=> "1"
    #  p responder.dialogue("") #=> "2"
    #  p responder.dialogue("") #=> "1"
    #
    class MemoryResponder < Base
...
if $0 == __FILE__
  # Albatro
  Responder::MemoryResponder.new.interactive(:messages => ["夢の中で会ったような…", "輪廻のこと", "夢の中とは？"])
  Albatro::Responder::MemoryResponder.new.interactive(:messages => ["世界卓球はじまるな", "輪廻のこと", "夢の中とは？"])
end
